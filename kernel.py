...
def main():
    if len(sys.argv) == 2:
        if sys.argv[1] >= "2.3.5":
            clrscr()
            print(term.bold("Procyon Neo Kernel 2.3.5"))
            print("..............")
            ...
            if os.path.exists("shell.py"):
                os.system("python3 shell.py 2.3.5")
            else:
                clrscr()
            ...
            print(term.red(f" Reported {sys.argv[1]} as opposed to 2.3.5"))
            sys.exit(1)
    else:
        clrscr()
        print(term.bold_red("Kernel Panic : Bootloader error"))
...
def callApplication(app, isAdmin=False):
    appResolved = f"python3 {app}.py 2.3.5 {isAdmin}"
    os.system(appResolved)

def callApplication3P(app, isAdmin=False):
    appResolved = f"python3 apps/{app}.py 2.3.5 {isAdmin}"
    os.system(appResolved)